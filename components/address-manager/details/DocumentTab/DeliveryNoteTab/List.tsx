...
const List = () => {
  ...
  const onPreviewPdf = async (delivery: any) => {
  ...
  const onDownloadPdf = async (delivery: any) => {
    const res = await previewPdf(delivery?._delivery_note_id, access_token);
    const blob = await res.blob();
    const objectURL = URL.createObjectURL(blob);
  ...
  return (
    <>
      <table className="w-full">
        ...
        <tbody>
          {Array.isArray(data?.dn) && data.dn.length === 0 && (
            <tr>
              <td colSpan={5}>
          ...
          {Array.isArray(data?.dn) &&
            data.dn.map((row: any, key: number) => (
      ...
      {data && data.pager && (
        <div className="mt-auto border-t-stone-100">
          <Pagination
            pager={data.pager}
