...
const List = ({ search }: { search?: any }) => {
  ...
  let { data, isLoading, error } = useSWR(
    `/api/cms/${cms?._cms_id}/order_confirmation?${queryString}`,
    fetcher,
    {
  ...
  const onPreviewPdf = async (oc: any) => {
    const res = await previewPdf(oc?._order_confirmation_id, access_token);
  ...
  const onDownloadPdf = async (oc: any) => {
    const res = await previewPdf(oc?._order_confirmation_id, access_token);
    const blob = await res.blob();
    const objectURL = URL.createObjectURL(blob);
  ...
  return (
    <>
      <table className="w-full">
        ...
        <tbody>
          {Array.isArray(data?.oc) && data.oc.length === 0 && (
            <tr>
              <td colSpan={5}>
          ...
          {Array.isArray(data?.oc) &&
            data.oc.map((row: any, key: number) => (
