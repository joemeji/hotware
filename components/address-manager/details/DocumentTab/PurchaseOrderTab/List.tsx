...
const List = () => {
  ...
  let { data, isLoading, error } = useSWR(
    `/api/cms/${cms?._cms_id}/purchase_order?${queryString}`,
    fetcher,
    {
  ...
  return (
    <>
      <table className="w-full">
        ...
        <tbody>
          {Array.isArray(data?.po) && data.po.length === 0 && (
            <tr>
              <td colSpan={5}>
          ...
          {Array.isArray(data?.po) &&
            data.po.map((row: any, key: number) => (
              <tr key={key} className="hover:bg-stone-100">
                ...
                <TD className="group-last:border-b-0 align-top">
                  <div className="flex flex-col gap-1">
                    <span className="font-medium">{row.po_description}</span>
