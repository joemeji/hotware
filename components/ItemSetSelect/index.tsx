...
const ItemSetSelect = (props: ItemSetSelectProps) => {
  ...
  const { data, isLoading, error, size, setSize, isValidating } =
    useSWRInfinite((index) => {
      ...
      if (shipping_id) param["_shipping_id"] = shipping_id;
  ...
  const onCheckItemId = (isChecked: boolean, item: any) => {
    ...
    const selectedItemIdIndex = _selectedItems.findIndex(
      (data: any) => data.item_set_id === item.item_set_id
    );
    if (selectedItemIdIndex > -1 && !isChecked)
      _selectedItems.splice(selectedItemIdIndex, 1);
  ...
  const getSingleSelectedItem = (item_set_id: string | any) => {
    const _selectedItem = selectedItems
      ? selectedItems.find((data: any) => data.item_set_id === item_set_id)
      : null;
          ...
          {_data &&
            ...
            _data.map((data: any) => {
              return (
                ...
                data.sets.map((item: any, key: number) => (
                  <label
                    htmlFor={item.item_set_id}
                    key={key}
                    className={cn(
                  ...
                  >
                    ...
                    <Checkbox
                      ...
                      checked={
                        getSingleSelectedItem(item.item_set_id)
                          ? getSingleSelectedItem(item.item_set_id)
