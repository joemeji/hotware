...
const PaymentTermSelect = (props: PaymentTermSelectProps) => {
  ...
  const { data, isLoading, error, mutate, size, setSize, isValidating } = useSWRInfinite((index) => {
    ...
    return [`/api/payment_terms?${searchParams}`, session?.user?.access_token];
  }, fetchApi);
              ...
              text: (
                ...
                >
                  <div className="flex gap-2 items-center justify-between w-full">
                    <span className="font-medium">{item.payment_terms_name}</span>
                  </div>
                </div>
              ),
