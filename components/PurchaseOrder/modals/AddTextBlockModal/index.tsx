...
function AddTextBlockModal(props: AddTextBlockModalProps) {
  ...
  const { open, onOpenChange, _po_id, onUpdated } = props;
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [selectedTextBlock, setSelectedTextBlock] = useState<any>(0);
  const { mutate } = useSWRConfig();

  const yupSchema = yup.object({
    potb_language: yup.string(),
    potb_title: yup.string(),
    potb_text: yup.string(),
    potb_extra_text: yup.string(),
  });
  ...
  const onSubmitEditForm = async (data: any) => {
    if (!data?.potb_text) return;

    try {
      setIsSubmitting(true);
      const response = await fetch(
        `${baseUrl}/api/purchases/text_blocks/${_po_id}`,
        {
      ...
      if (json.success) {
        mutate([
          `/api/projects/purchases/text_blocks/${_po_id}`,
  ...
  const onChangeTextBlock = (textBlock: any) => {
    ...
    setValue("potb_title", textBlock.text_block_title);
    setValue("potb_text", textBlock.text_block_text);
    setValue("potb_extra_text", textBlock.text_block_extra_text);
  };
              ...
              <div className="flex flex-col gap-2">
                ...
                <div>
                  <Combobox
                    ...
                    value="English"
                    onChangeValue={(value) => setValue("potb_language", value)}
                  />
                </div>
              </div>
              <div className="flex flex-col gap-2">
