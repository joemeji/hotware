...
function EditPurchaseItemModal(props: EditPurchaseItemModalProps) {
  const { data: session }: any = useSession();
  const { open, onOpenChange, po, _po_id, onUpdated } = props;
  ...
  const onSubmitEditForm = async (e: any) => {
    ...
    try {
      ...
      const response = await fetch(
        `${baseUrl}/api/purchases/items/update/${po.po_item_id}`,
        {
          headers: authHeaders(session?.user?.access_token),
          method: "POST",
          body: JSON.stringify({ data: { po_item_name: newValue } }),
        }
      );
      ...
      if (json.success) {
        ...
        if (onUpdated) onUpdated(po.po_item_id, newValue);
        ...
        <form onSubmit={onSubmitEditForm}>
          <div className="flex flex-col gap-3 p-4 relative min-h-[560px]">
            <div className="w-full h-[460px]">
              <RichTextEditor ref={editorRef} value={po?.po_item_name} />
            </div>
          </div>
...
export default memo(EditPurchaseItemModal);

type EditPurchaseItemModalProps = {
  open?: boolean;
