import {
  ...
  function AddTextBlockAttachmentModal(props: AddTextBlockAttachmentModalProps) {
    ...
    const { open, onOpenChange, potb_id, onUpdated } = props;
    ...
    const { mutateDelete, Dialog: DeleteDialog } = useDeleteTextBlockAttachment({
      potb_id,
      onDelete: (potb_attachment_id: string) => {
        const updatedList = items.filter(
          (item) => item.potb_attachment_id !== potb_attachment_id
        );
    ...
    const onSubmitEditForm = async (data: any) => {
      try {
        ...
        let endpoint = `${baseUrl}/api/purchases/text_blocks/attachments/${potb_id}/null`;
        ...
        if (isEditMode && editItem) {
          ...
          endpoint = `${baseUrl}/api/purchases/text_blocks/attachments/${editItem.potb_id}/${editItem.potb_attachment_id}`;
        }
        ...
        if (json.success) {
          ...
          mutate([
            `/api/purchases/text_blocks/attachments/${potb_id}`,
                        ...
                        <tr key={key} className="group">
                          ...
                          <TD className="align-top text-right pe-4">
                            <MoreOption>
                              ...
                              <div
                                onClick={() =>
                                  mutateDelete(row.potb_attachment_id)
                                }
                                className="flex items-center p-2 px-3 cursor-pointer gap-3 hover:bg-stone-100 outline-none"
                              >
