...
function AddReceiptTotalModal(props: AddReceiptTotalModalProps) {
  ...
  const onSubmitEditForm = async (e: any) => {
    ...
    const filteredSurcharges = surcharges
      .filter((surcharge) => surcharge.port_value && surcharge.port_text)
      .map((surcharge) => ({ ...surcharge, port_is_surcharge: 1 }));
    const filteredDeductions = deductions
      .filter((deduction) => deduction.port_value && deduction.port_text)
      .map((surcharge) => ({ ...surcharge, port_is_surcharge: 0 }));
    ...
    try {
      ...
      const response = await fetch(
        `${baseUrl}/api/purchases/receipt_totals/${_po_id}`,
        {
      ...
      if (json.success) {
        ...
        mutate([
          `/api/purchases/receipt_totals/${_po_id}`,
          session?.user?.access_token,
        ]);
      }
    } catch (err: any) {
  ...
  useEffect(() => {
    if (data && data.length) {
      const existingSurcharges = data.filter(
        (receiptTotal: any) => receiptTotal.port_is_surcharge == 1
      );
                    ...
                    <tr key={index}>
                      <td className="w-[10%] border border-[#e4e7ea] p-2">
                        <ReceiptTotalTypeSelect
                          modal={true}
                          value={surcharge.port_type}
                          onChangeValue={(value) =>
                            onChangeSurcharge(index, { port_type: value })
                          }
                        />
                      </td>
                      <td
                      ...
                      <td className="w-[10%] border border-[#e4e7ea] p-2">
                        <VatSelect
                          modal={true}
                          value={surcharge.port_vat}
                          onChangeValue={(value) =>
                            onChangeSurcharge(index, { port_vat: value })
                          }
                        />
                      </td>
                      <td className="w-[10%] border border-[#e4e7ea] p-2">
                  ...
                  {deductions.map((deduction, index) => (
                    <tr key={index}>
                      <td className="w-[10%] border border-[#e4e7ea] p-2">
                        <ReceiptTotalTypeSelect
                          modal={true}
                          value={deduction.port_type}
                          onChangeValue={(value) =>
                            onChangeDeduction(index, { port_type: value })
                          }
                        />
                      </td>
                      <td
                      ...
                      <td className="w-[10%] border border-[#e4e7ea] p-2">
                        <VatSelect
                          modal={true}
                          value={deduction.port_vat}
                          onChangeValue={(value) =>
                            onChangeDeduction(index, { port_vat: value })
                          }
                        />
                      </td>
                      <td className="w-[10%] border border-[#e4e7ea] p-2">
