...
function EditPurchaseDetails({ id, purchase_order }: any) {
  ...
  const onSubmitForm = async (data: any) => {
    ...
    try {
      ...
      const res = await fetch(`${baseUrl}/api/purchases/update/${id}`, {
        method: "POST",
      ...
      if (json.success) {
        toast({
          title: "A purchase order has been updated successfully.",
          variant: "success",
          duration: 2000,
        });
        setTimeout(() => {
    ...
    <form onSubmit={handleSubmit(onSubmitForm)}>
      <div className="flex flex-col p-5 gap-3 w-full mx-auto max-w-[1600px]">
        <div className="flex justify-between bg-background p-3 rounded-app items-center shadow-sm">
          <h1 className="text-lg font-medium">Edit Purchase Order</h1>
...
export default memo(EditPurchaseDetails);
