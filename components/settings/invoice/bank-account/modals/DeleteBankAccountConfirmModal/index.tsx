...
export const DeleteBankAccountConfirmModal = (props: BankAccountModal) => {
  const { open, onOpenChange, data, listUrl } = props;

  const deleteBankAccount = async () => {
    try {
      const res = await fetch("/api/invoice/bank-account/delete", {
        method: "POST",
        body: JSON.stringify({
          id: data.ba_id,
        }),
      ...
      <div className='pb-5 px-5'>
        <div className='flex flex-col gap-3 justify-center items-center'>
          ...
          <p className='text-lg w-1/2 text-center'>
            Are you sure you want to delete {data?.ba_account_name} ? You
          ...
          <div className='flex gap-2'>
            ...
            <Button variant='red' onClick={deleteBankAccount}>
              Yes
            </Button>
          </div>
        </div>
