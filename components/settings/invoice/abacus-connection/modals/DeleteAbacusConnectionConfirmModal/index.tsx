...
export const DeleteAbacusConnectionConfirmModal = (props: AbacusConnectionModal) => {
  const { open, onOpenChange, data, listUrl } = props;

  const deleteAbacusConnection = async () => {
    try {
      const res = await fetch("/api/invoice/abacus-connection/delete", {
        method: "POST",
        body: JSON.stringify({
          id: data.company_id,
        }),
      ...
      <div className='pb-5 px-5'>
        <div className='flex flex-col gap-3 justify-center items-center'>
          ...
          <p className='text-lg w-1/2 text-center'>
            Are you sure you want to delete abacus connection for {data?.company_name} ? You
          ...
          <div className='flex gap-2'>
            ...
            <Button variant='red' onClick={deleteAbacusConnection}>
              Yes
            </Button>
          </div>
        </div>
