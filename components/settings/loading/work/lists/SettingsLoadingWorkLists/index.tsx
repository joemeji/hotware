...
import { AddLoadingWorkModal } from "../../modals/AddLoadingWorkModal";
import { DeleteLoadingWorkConfirmModal } from "../../modals/DeleteLoadingWorkConfirmModal";
import { PER_PAGE } from "@/utils/algoliaConfig";

const SettingsLoadingWorkLists = () => {
  ...
  const listUrl = `/api/loading/work/paginate?page=${page}&search=${searchText}`;
  ...
  const loading_work = data?.loading_work;
    ...
    <div className='grid bg-white'>
      ...
      <div className={cn("bg-white p-7", {})}>
        <div className='flex items-center justify-between '>
          <h1 className='text-2xl font-light mb-5'>Manage Loading Work</h1>
      ...
      <div className='p-3'>
        <div className='min-h-full'>
          <table className='w-full rounded-sm overflow-hidden p-5'>
            ...
            <tbody>
              {loading_work &&
                loading_work.length > 0 &&
                loading_work.map((type: any, i: number) => {
                  return (
                    <tr key={i} className='text-center &_td:border-r'>
                      <TD>{(page - 1) * PER_PAGE + i + 1}</TD>
                      <TD className='text-left'>
                        {type.loading_work_name}
                      </TD>
                      <TD>
                        <ActionMenu
          ...
          {!isLoading && loading_work?.length == 0 && (
...
export default memo(SettingsLoadingWorkLists);
