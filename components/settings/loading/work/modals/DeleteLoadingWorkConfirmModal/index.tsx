...
export const DeleteLoadingWorkConfirmModal = (props: LoadingWorkModal) => {
  const { open, onOpenChange, data, listUrl } = props;

  const deleteLoadingWork = async () => {
    try {
      const res = await fetch("/api/loading/work/delete", {
        method: "POST",
        body: JSON.stringify({
      ...
      <div className='pb-5 px-5'>
        <div className='flex flex-col gap-3 justify-center items-center'>
          ...
          <p className='text-lg w-1/2 text-center'>
            Are you sure you want to delete {data?.loading_work_name} ? You
          ...
          <div className='flex gap-2'>
            ...
            <Button variant='red' onClick={deleteLoadingWork}>
              Yes
            </Button>
          </div>
        </div>
