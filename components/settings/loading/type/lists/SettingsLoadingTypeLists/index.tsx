...
import { AddLoadingTypeModal } from "../../modals/AddLoadingTypeModal";
import { DeleteLoadingTypeConfirmModal } from "../../modals/DeleteLoadingTypeConfirmModal";
import { PER_PAGE } from "@/utils/algoliaConfig";

const SettingsLoadingTypeLists = () => {
  ...
  const listUrl = `/api/loading/type/paginate?page=${page}&search=${searchText}`;
  ...
  const loading_type = data?.loading_type;
    ...
    <div className='grid bg-white'>
      ...
      <div className={cn("bg-white p-7", {})}>
        <div className='flex items-center justify-between '>
          <h1 className='text-2xl font-light mb-5'>Manage Loading Type</h1>
      ...
      <div className='p-3'>
        <div className='min-h-full'>
          <table className='w-full rounded-sm overflow-hidden p-5'>
            ...
            <tbody>
              {loading_type &&
                loading_type.length > 0 &&
                loading_type.map((type: any, i: number) => {
                  return (
                    <tr key={i} className='text-center &_td:border-r'>
                      <TD>{(page - 1) * PER_PAGE + i + 1}</TD>
                      <TD className='text-left'>
                        {type.loading_type_name}
                      </TD>
                      <TD>
                        <ActionMenu
          ...
          {!isLoading && loading_type?.length == 0 && (
...
export default memo(SettingsLoadingTypeLists);
