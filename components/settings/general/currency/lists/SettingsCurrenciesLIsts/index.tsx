import { memo, useEffect, useState } from "react";
import { ActionMenu, TD, TH, tblCurrenciesHeadings } from "../..";
import { fetcher } from "@/utils/api.config";
import useSWR, { mutate } from "swr";
import { useRouter } from "next/router";
import { Loader2, Plus } from "lucide-react";
import { Button } from "@/components/ui/button";
import { iconProps } from "@/components/admin-layout/sidebar/general-settings/general-settings-menu";
import { Input } from "@/components/ui/input";
import { cn } from "@/lib/utils";
import Pagination from "@/components/pagination";
import { CurrencyModal } from "../../modals/CurrencyModal";
import { toast } from "@/components/ui/use-toast";
import { DeleteCurrencyConfirmModal } from "../../modals/DeleteCurrencyConfirmModal";
import { PER_PAGE } from "@/utils/algoliaConfig";

const SettingsCurrenciesLists = () => {
  const router = useRouter();

  const [searchText, setSearchText] = useState("");
  const [page, setPage] = useState(1);

  const onPaginate = (page: any) => {
    setPage(page);
    router.query.page = page;
    router.push(router);
  };

  const listUrl = `/api/currency/paginate?page=${page}&search=${searchText}`;

  const { data, isLoading } = useSWR(listUrl, fetcher, {
    revalidateOnFocus: false,
    revalidateIfStale: false,
  });

  const currencies = data?.currencies;
  const pager = data?.pager;

  const handleDelete = async (id: string, currencyName : string) => {

    setSelectedId(id)
    setSelectedCurrency(currencyName)
    setOpenDeleteConfirm(true)
  };

  const handleEdit = (id : string) => {
    setSelectedId(id)
    setOpenCurrencyModal(true)
  };

  const [selectedId, setSelectedId] = useState('')
  const [openCurrencyModal, setOpenCurrencyModal] = useState(false);
  const [openDeleteConfirm, setOpenDeleteConfirm] = useState(false);
  const [selectedCurrency, setSelectedCurrency] = useState('')

  return (
    <div className='grid bg-white'>
      <CurrencyModal
        open={openCurrencyModal}
        id={selectedId}
        listUrl={listUrl}
        onOpenChange={(open: any) => setOpenCurrencyModal(open)}
      />
      <DeleteCurrencyConfirmModal
        open={openDeleteConfirm}
        id={selectedId}
        currencyName={selectedCurrency}
        listUrl={listUrl}
        onOpenChange={(open: any) => setOpenDeleteConfirm(open)}
      />
      <div className={cn("bg-white p-7", {})}>
        <div className='flex items-center justify-between '>
          <h1 className='text-2xl font-light mb-5'>Manage Currencies</h1>
          <div className='flex gap-2'>
            <div>
              <Input
                placeholder='Search'
                onChange={(e) => setSearchText(e.target.value)}
              />
            </div>
            <Button
              variant='red'
              onClick={() => {
                setOpenCurrencyModal(true);
                setSelectedId('')
              }}
            >
              <Plus {...iconProps} />
              Add Currency
            </Button>
          </div>
        </div>
      </div>
      <div className='p-3'>
        <div className='min-h-full'>
          <table className='w-full rounded-sm overflow-hidden p-5'>
            <thead>
              <tr>
                {tblCurrenciesHeadings.map((heading: any, i: any) => {
                  return (
                    <TH key={i} className={heading?.class}>
                      {heading?.name}
                    </TH>
                  );
                })}
              </tr>
            </thead>
            <tbody>
              {currencies &&
                currencies.length > 0 &&
                currencies.map((currency: any, i: number) => {
                  return (
                    <tr key={i}>
                      <TD>{(page - 1) * PER_PAGE + i + 1}</TD>
                      <TD>{currency.currency}</TD>
                      <TD>{currency.currency_name}</TD>
                      <TD>{currency.currency_sign}</TD>
                      <TD>{currency.CHF_value}</TD>
                      <TD>
                        <ActionMenu
                          onDelete={() =>
                            handleDelete(
                              currency.currency_id,
                              currency.currency_name
                            )
                          }
                          onEdit={() => handleEdit(currency.currency_id)}
                          data={{
                            id: currency.currency_id,
                          }}
                        />
                      </TD>
                    </tr>
                  );
                })}
            </tbody>
          </table>
          {!isLoading && currencies?.length == 0 && (
            <div className='text-center max-w-full p-5'>No records found</div>
          )}

          {isLoading && <Loader2 className='animate-spin mx-auto m-5' />}

          {pager && (
            <div className='mt-auto border-t border-t-stone-100 flex justify-end'>
              <Pagination pager={pager} onPaginate={onPaginate} />
            </div>
          )}
        </div>
      </div>
    </div>
  );
};

export default memo(SettingsCurrenciesLists);

// interface ISettingsCurrenciesLIsts  {

// }
