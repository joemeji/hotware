...
export const DeletePaymentTermsConfirmModal = (props: IPaymentTermsModal) => {
  ...
  const deletePaymentTerms = async() => {
    ...
    try {
      const res = await fetch(
        "/api/payment-terms/delete",
        {
          method: "POST",
          body: JSON.stringify({
            id: data.payment_terms_id
          })
        },
      );
      ...
      <div className='pb-5 px-5'>
        <div className='flex flex-col gap-3 justify-center items-center'>
          ...
          <p className="text-lg w-1/2 text-center">
            Are you sure you want to delete {data?.payment_terms_name} ?
          </p>
          <div className='flex gap-2'>
            <Button variant='default' onClick={() => onOpenChange && onOpenChange(false)}>Cancel</Button>
            <Button variant='red' onClick={deletePaymentTerms}>Yes</Button>
          </div>
        </div>
      </div>
    </GenericModal>
