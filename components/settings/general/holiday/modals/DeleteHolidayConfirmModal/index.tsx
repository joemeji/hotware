...
export const DeleteHolidayConfirmModal = (props: IHolidayModal) => {
  const { open, onOpenChange, listUrl, holiday  } = props;
      ...
      <div className='pb-5 px-5'>
        <div className='flex flex-col gap-3 justify-center items-center'>
          ...
          <p className="text-lg w-1/2 text-center">
            Are you sure you want to delete {holiday?.holiday_name}? You won&apos;t be able to
            revert this.
          </p>
          <div className='flex gap-2'>
            <Button variant='default' onClick={() => onOpenChange && onOpenChange(false)}>Cancel</Button>
            <Button variant='red' onClick={deleteCountry}>Yes</Button>
          </div>
        </div>
      </div>
    </GenericModal>
