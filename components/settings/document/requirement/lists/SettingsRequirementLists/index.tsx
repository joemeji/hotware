...
import { DeleteRequirementConfirmModal } from "../../modals/DeleteRequirementConfirmModal";
...
const SettingsRequirementLists = () => {
  ...
  const listUrl = `/api/document/requirement/paginate?type=${type}&page=${page}&search=${searchText}`;
  ...
  const document_category = data?.document_category;
  ...
  const handleTypeChange = (value: any) => {
    setType(value);
    // Trigger re-fetch when type changes
    mutate(listUrl);
  }
    ...
    <div className='grid bg-white'>
      ...
      <div className={cn("bg-white p-7", {})}>
        <div className='flex items-center justify-between '>
          <h1 className='text-2xl font-light mb-5'>Manage Document Categories</h1>
      ...
      <div className='p-3'>
        <div className='min-h-full'>
          <table className='w-full rounded-sm overflow-hidden p-5'>
            ...
            <tbody>
              {document_category &&
                ...
                document_category.map((type: any, i: number) => {
                  return (
                    <tr key={i} className='&_td:border-r'>
                      <TD>{(page - 1) * PER_PAGE + i + 1}</TD>
                      <TD className='text-left'>
                        {type.document_category_name}
                      </TD>
                      <TD className='text-center'>
                        {type.document_type_name?.toUpperCase()}
                      </TD>
          ...
          {!isLoading && document_category?.length == 0 && (
...
export default memo(SettingsRequirementLists);
