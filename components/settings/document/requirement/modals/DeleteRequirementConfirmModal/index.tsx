...
export const DeleteRequirementConfirmModal = (props: RequirementModal) => {
  const { open, onOpenChange, data, listUrl } = props;

  const deleteRequirement = async () => {
    try {
      const res = await fetch("/api/document/requirement/delete", {
        method: "POST",
        body: JSON.stringify({
      ...
      <div className='pb-5 px-5'>
        <div className='flex flex-col gap-3 justify-center items-center'>
          ...
          <p className='text-lg w-1/2 text-center'>
            Are you sure you want to delete {data?.document_category_name} ? You
          ...
          <div className='flex gap-2'>
            ...
            <Button variant='red' onClick={deleteRequirement}>
              Yes
            </Button>
          </div>
        </div>
