...
export const DeleteRiskManagementModalConfirmModal = (
  props: IDeleteRiskManagementModalConfirmModal
) => {
  const { open, onOpenChange, data, listUrl } = props;

  const deleteCurrency = async () => {
    try {
      const res = await fetch("/api/document/risk-management/delete", {
        method: "POST",
        body: JSON.stringify({
      ...
      <div className='pb-5 px-5'>
        <div className='flex flex-col gap-3 justify-center items-center'>
          ...
          <p className='text-lg w-1/2 text-center'>
            Are you sure you want to delete {data?.risk_management_name} ? You
...
type IDeleteRiskManagementModalConfirmModal = {
  open?: boolean;
  data?: any;
  listUrl: string;
