...
const SettingsRiskManagementLists = () => {
  ...
  const listUrl = `/api/document/risk-management/paginate?page=${page}&search=${searchText}`;
  ...
  const riskManagment = data?.risk_managements;
  ...
  return (
    <div className='grid bg-white'>
      <AddRiskManagementModal
        open={openAddModal}
        data={selectedData}
        listUrl={listUrl}
        onOpenChange={(open: any) => setOpenAddModal(open)}
      />
      <DeleteRiskManagementModalConfirmModal
      ...
      <div className={cn("bg-white p-7", {})}>
        <div className='flex items-center justify-between '>
          <h1 className='text-2xl font-light mb-5'>Manage Risk Management</h1>
      ...
      <div className='p-3'>
        <div className='min-h-full'>
          <table className='w-full rounded-sm overflow-hidden p-5'>
            ...
            <tbody>
              {riskManagment &&
                riskManagment.length > 0 &&
                riskManagment.map((textBlock: any, i: number) => {
                  return (
                    <tr key={i} className=''>
                      ...
                      <TD className='text-center'>
                        <ActionMenu
                          onDelete={() => handleDelete(textBlock)}
                          onEdit={() => handleEdit(textBlock)}
                          data={{
                            id: textBlock.risk_management_id,
                          }}
                        />
                      </TD>
                    </tr>
          ...
          {!isLoading && riskManagment?.length == 0 && (
...
export default memo(SettingsRiskManagementLists);
