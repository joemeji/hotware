...
const yupObject: any = {
  user_skill_name: yup.string().required("This field is required."),
};

export const UpdateSkill = (props: UpdateSkillProps) => {
  const access_token = useContext(AccessTokenContext);
  const router = useRouter();
  const { open, onOpenChange, onSuccess, dir } = props;
  ...
  const handleFormSubmit = async (data: any) => {
    try {
      ...
      data.user_skill_id = dir.usc_id;

      setLoading(true);
      const res = await fetch(`${baseUrl}/api/users/service/update_skill`, {
        method: "POST",
          ...
          >
            <div className="flex flex-col gap-3 mt-2 px-1">
              ...
              <Input
                ...
                error={errors && (errors.user_skill_name ? true : false)}
                {...register("user_skill_name")}
              />
              {errors.user_skill_name && (
                <span className="text-red-500 text-sm">
                  <>{errors.user_skill_name?.message}</>
                </span>
              )}
              <DialogFooter>
                <Button
