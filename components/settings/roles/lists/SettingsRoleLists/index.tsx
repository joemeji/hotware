...
import { DeleteRoleConfirmModal } from "../../modals/DeleteRoleConfirmModal";
import { AddRoleModuleModal } from "../../modals/AddRoleModuleModal";
import { PER_PAGE } from "@/utils/algoliaConfig";

const SettingsRoleLists = () => {
  ...
  const listUrl = `/api/roles/paginate?page=${page}&search=${searchText}`;
  ...
  const handleAddRole = (data: any) => {
    setSelectedData(data);
    setOpenAddRoleModule(true);
  };

  const [openAddModal, setOpenAddModal] = useState(false);
  const [openDeleteConfirm, setOpenDeleteConfirm] = useState(false);
  const [openAddRoleModule, setOpenAddRoleModule] = useState(false);
  ...
  return (
    <div className='grid bg-white'>
      ...
      />
      <AddRoleModuleModal
        open={openAddRoleModule}
        data={selectedData}
        onOpenChange={setOpenAddRoleModule}
      />
      <div className={cn("bg-white p-7", {})}>
        <div className='flex items-center justify-between '>
          <h1 className='text-2xl font-light mb-5'>Manage Roles</h1>
...
export default memo(SettingsRoleLists);
