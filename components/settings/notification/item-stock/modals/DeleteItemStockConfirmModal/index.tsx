...
export const DeleteConfirmModal = (props: ItemStockModal) => {
  const { open, onOpenChange, data, listUrl } = props;

  const deleteItemStock = async () => {
    try {
      const res = await fetch("/api/notification/item-stock/delete", {
        method: "POST",
        body: JSON.stringify({
      ...
      <div className='pb-5 px-5'>
        <div className='flex flex-col gap-3 justify-center items-center'>
          ...
          <div className='flex gap-2'>
            ...
            <Button variant='red' onClick={deleteItemStock}>
              Yes
            </Button>
          </div>
        </div>
