...
import { dueInvoicesSchema } from "../../schema";
import { useEffect } from "react";
import { TechnicianSelect } from "@/components/app/technician-select";

interface DueInvoicesForm {
  id?: string;
  listUrl: string;
  onOpenChange?: (open: boolean) => void;
}
export const DueInvoicesForm = (props: DueInvoicesForm) => {
  const { id, listUrl, onOpenChange } = props;

  const { data, isLoading } = useSWR(
    id ? `/api/notification/due-invoices/info?id=${id}` : null,
    fetcher
  );
  ...
  const {
    ...
    resolver: yupResolver(dueInvoicesSchema),
  });
  ...
  const submit = async (data: any) => {
    try {
      ...
      const url = "/api/notification/due-invoices" + (id
        ? "/update"
        : "/create");
