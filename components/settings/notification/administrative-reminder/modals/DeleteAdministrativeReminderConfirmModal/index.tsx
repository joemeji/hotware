...
export const DeleteConfirmModal = (props: AdministrativeReminderModal) => {
  const { open, onOpenChange, data, listUrl } = props;

  const deleteAdministrativeReminder = async () => {
    try {
      const res = await fetch("/api/notification/administrative-reminder/delete", {
        method: "POST",
        body: JSON.stringify({
      ...
      <div className='pb-5 px-5'>
        <div className='flex flex-col gap-3 justify-center items-center'>
          ...
          <p className='text-lg w-1/2 text-center'>
            Are you sure you want to delete {data?.administrative_reminder_initial} ? You
          ...
          <div className='flex gap-2'>
            ...
            <Button variant='red' onClick={deleteAdministrativeReminder}>
              Yes
            </Button>
          </div>
        </div>
...
type AdministrativeReminderModal = {
  open?: boolean;
  data?: any;
  listUrl: string;
