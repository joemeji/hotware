...
import { AddLeaveReminderModal } from "../../modals/AddLeaveReminderModal";
import { DeleteConfirmModal } from "../../modals/DeleteLeaveReminderConfirmModal";
import { PER_PAGE } from "@/utils/algoliaConfig";

const SettingsLeaveReminderLists = () => {
  ...
  const listUrl = `/api/notification/leave-reminder/paginate?page=${page}&search=${searchText}`;
  ...
  const leave_reminder = data?.leave_reminder;
    ...
    <div className='grid bg-white'>
      ...
      <div className={cn("bg-white p-7", {})}>
        <div className='flex items-center justify-between '>
          <h1 className='text-2xl font-light mb-5'>Manage Leave Reminders</h1>
      ...
      <div className='p-3'>
        <div className='min-h-full'>
          <table className='w-full rounded-sm overflow-hidden p-5'>
            ...
            <tbody>
              {leave_reminder &&
                leave_reminder.length > 0 &&
                leave_reminder.map((type: any, i: number) => {
                  return (
          ...
          {!isLoading && leave_reminder?.length == 0 && (
...
export default memo(SettingsLeaveReminderLists);
