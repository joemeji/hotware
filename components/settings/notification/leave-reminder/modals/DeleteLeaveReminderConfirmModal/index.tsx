...
export const DeleteConfirmModal = (props: LeaveReminderModal) => {
  const { open, onOpenChange, data, listUrl } = props;

  const deleteLeaveReminder = async () => {
    try {
      const res = await fetch("/api/notification/leave-reminder/delete", {
        method: "POST",
        body: JSON.stringify({
      ...
      <div className='pb-5 px-5'>
        <div className='flex flex-col gap-3 justify-center items-center'>
          ...
          <div className='flex gap-2'>
            ...
            <Button variant='red' onClick={deleteLeaveReminder}>
              Yes
            </Button>
          </div>
        </div>
