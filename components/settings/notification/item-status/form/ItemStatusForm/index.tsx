...
import { itemStatusSchema } from "../../schema";
import { useEffect } from "react";
import { TechnicianSelect } from "@/components/app/technician-select";

interface ItemStatusForm {
  id?: string;
  listUrl: string;
  onOpenChange?: (open: boolean) => void;
}
export const ItemStatusForm = (props: ItemStatusForm) => {
  const { id, listUrl, onOpenChange } = props;

  const { data, isLoading } = useSWR(
    id ? `/api/notification/item-status/info?id=${id}` : null,
    fetcher
  );
  ...
  const {
    ...
    resolver: yupResolver(itemStatusSchema),
  });
  ...
  const submit = async (data: any) => {
    try {
      ...
      const url = "/api/notification/item-status" + (id
        ? "/update"
        : "/create");
