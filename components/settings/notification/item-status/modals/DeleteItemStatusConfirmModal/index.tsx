...
export const DeleteConfirmModal = (props: ItemStatusModal) => {
  const { open, onOpenChange, data, listUrl } = props;

  const deleteItemStatus = async () => {
    try {
      const res = await fetch("/api/notification/item-status/delete", {
        method: "POST",
        body: JSON.stringify({
      ...
      <div className='pb-5 px-5'>
        <div className='flex flex-col gap-3 justify-center items-center'>
          ...
          <div className='flex gap-2'>
            ...
            <Button variant='red' onClick={deleteItemStatus}>
              Yes
            </Button>
          </div>
        </div>
