...
import { technicianDocumentSchema } from "../../schema";
import { useEffect } from "react";
import { TechnicianSelect } from "@/components/app/technician-select";

interface TechnicianDocumentForm {
  id?: string;
  listUrl: string;
  onOpenChange?: (open: boolean) => void;
}
export const TechnicianDocumentForm = (props: TechnicianDocumentForm) => {
  const { id, listUrl, onOpenChange } = props;

  const { data, isLoading } = useSWR(
    id ? `/api/notification/technician-document/info?id=${id}` : null,
    fetcher
  );
  ...
  const {
    ...
    resolver: yupResolver(technicianDocumentSchema),
  });
  ...
  const submit = async (data: any) => {
    try {
      ...
      const url = "/api/notification/technician-document" + (id
        ? "/update"
        : "/create");
