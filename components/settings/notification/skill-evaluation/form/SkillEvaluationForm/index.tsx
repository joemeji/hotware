...
import { skillEvaluationSchema } from "../../schema";
import { useEffect } from "react";
import { TechnicianSelect } from "@/components/app/technician-select";

interface SkillEvaluationForm {
  id?: string;
  listUrl: string;
  onOpenChange?: (open: boolean) => void;
}
export const SkillEvaluationForm = (props: SkillEvaluationForm) => {
  const { id, listUrl, onOpenChange } = props;

  const { data, isLoading } = useSWR(
    id ? `/api/notification/skill-evaluation/info?id=${id}` : null,
    fetcher
  );
  ...
  const {
    ...
    resolver: yupResolver(skillEvaluationSchema),
  });
  ...
  const submit = async (data: any) => {
    try {
      ...
      const url = "/api/notification/skill-evaluation" + (id
        ? "/update"
        : "/create");
