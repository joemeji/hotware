...
import { AddSkillEvaluationModal } from "../../modals/AddSkillEvaluationModal";
import { DeleteConfirmModal } from "../../modals/DeleteSkillEvaluationConfirmModal";
import { PER_PAGE } from "@/utils/algoliaConfig";

const SettingsSkillEvaluationLists = () => {
  ...
  const listUrl = `/api/notification/skill-evaluation/paginate?page=${page}&search=${searchText}`;
  ...
  const skill_evaluation = data?.skill_evaluation;
    ...
    <div className='grid bg-white'>
      ...
      <div className={cn("bg-white p-7", {})}>
        <div className='flex items-center justify-between '>
          <h1 className='text-2xl font-light mb-5'>Manage Skill Evaluation Reminders</h1>
      ...
      <div className='p-3'>
        <div className='min-h-full'>
          <table className='w-full rounded-sm overflow-hidden p-5'>
            ...
            <tbody>
              {skill_evaluation &&
                skill_evaluation.length > 0 &&
                skill_evaluation.map((type: any, i: number) => {
                  return (
          ...
          {!isLoading && skill_evaluation?.length == 0 && (
...
export default memo(SettingsSkillEvaluationLists);
