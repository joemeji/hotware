...
export const DeletePurchaseOrderNotifierConfirmModal = (
  props: DeletePurchaseOrderNotifierConfirmModal
) => {
  const { open, onOpenChange, data, listUrl } = props;

  const deleteNotifier = async () => {
    try {
      const res = await fetch("/api/notification/purchase-order/notifier/delete", {
        method: "POST",
        body: JSON.stringify({
          id: data.po_notify_id,
        }),
      });
      ...
      <div className='pb-5 px-5'>
        <div className='flex flex-col gap-3 justify-center items-center'>
          ...
          <p className='text-lg w-1/2 text-center'>
            Are you sure you want to delete <b>{data?.user_firstname} {data?.user_lastname}</b> as notifier?
          ...
          <div className='flex gap-2'>
            ...
            <Button variant='red' onClick={deleteNotifier}>
              Yes
            </Button>
          </div>
        </div>
...
type DeletePurchaseOrderNotifierConfirmModal = {
  open?: boolean;
  data?: any;
  listUrl: string;
