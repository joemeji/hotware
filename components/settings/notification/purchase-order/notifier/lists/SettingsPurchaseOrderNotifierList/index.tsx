...
import { AddPurchaseOrderNotifierModal } from "../../modals/AddPurchaseOrderNotifierModal";
import { DeletePurchaseOrderNotifierConfirmModal } from "../../modals/DeletePurchaseOrderNotifierConfirmModal";
import { PER_PAGE } from "@/utils/algoliaConfig";

interface SettingsPurchaseOrderNotifierList {
  companySettingValue?: boolean;
  setCompanySettingValue?: any;
}

const SettingsPurchaseOrderNotifierList = (
  props: SettingsPurchaseOrderNotifierList
) => {
  ...
  const listUrl = `/api/notification/purchase-order/notifier/paginate?page=${page}&search=${searchText}`;
  ...
  return (
    <div className='grid bg-white'>
      <AddPurchaseOrderNotifierModal
        open={openAddModal}
        data={selectedData}
        listUrl={listUrl}
        onOpenChange={(open: any) => setOpenAddModal(open)}
      />
      <DeletePurchaseOrderNotifierConfirmModal
        ...
        <div className='min-h-full'>
          <table className='w-full rounded-sm overflow-hidden p-5'>
            ...
            <tbody>
              {notifier &&
                notifier.length > 0 &&
                notifier.map((role: any, i: number) => {
                  return (
                    <tr key={i} className='text-center &_td:border-r'>
                      <TD>{(page - 1) * PER_PAGE + i + 1}</TD>
                      <TD className='text-left'>{role.user_firstname} {role.user_lastname}</TD>
                      <TD className='text-left'>{role.user_email_address}</TD>
                      <TD>
                        <ActionMenu
                          onDelete={() => handleDelete(role)}
                          data={{
                            id: role.po_notify_id,
                          }}
                        />
                      </TD>
                    </tr>
...
export default memo(SettingsPurchaseOrderNotifierList);
