...
export const DeletePurchaseOrderApproverConfirmModal = (
  props: DeletePurchaseOrderApproverConfirmModal
) => {
  const { open, onOpenChange, data, listUrl } = props;

  const deleteApprover = async () => {
    try {
      const res = await fetch("/api/notification/purchase-order/approver/delete", {
        method: "POST",
        body: JSON.stringify({
      ...
      <div className='pb-5 px-5'>
        <div className='flex flex-col gap-3 justify-center items-center'>
          ...
          <p className='text-lg w-1/2 text-center'>
            Are you sure you want to delete <b>{data?.user_firstname} {data?.user_lastname}</b> as approver? You
          ...
          <div className='flex gap-2'>
            ...
            <Button variant='red' onClick={deleteApprover}>
              Yes
            </Button>
          </div>
        </div>
...
type DeletePurchaseOrderApproverConfirmModal = {
  open?: boolean;
  data?: any;
  listUrl: string;
