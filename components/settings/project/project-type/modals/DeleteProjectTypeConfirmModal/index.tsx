...
export const DeleteProjectTypeConfirmModal = (props: ProjectTypeModal) => {
  const { open, onOpenChange, data, listUrl } = props;

  const deleteProjectType = async () => {
    try {
      const res = await fetch("/api/project/project-type/delete", {
        method: "POST",
        body: JSON.stringify({
      ...
      <div className='pb-5 px-5'>
        <div className='flex flex-col gap-3 justify-center items-center'>
          ...
          <p className='text-lg w-1/2 text-center'>
            Are you sure you want to delete {data?.project_type_name} ? You
          ...
          <div className='flex gap-2'>
            ...
            <Button variant='red' onClick={deleteProjectType}>
              Yes
            </Button>
          </div>
        </div>
