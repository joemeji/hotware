...
export const DeleteRequirementLevelConfirmModal = (props: IDepartmentModal) => {
  const { open, onOpenChange, data, listUrl } = props;

  const deleteCurrency = async () => {
    try {
      const res = await fetch("/api/cms/requirement-level/delete", {
        method: "POST",
        body: JSON.stringify({
      ...
      <div className='pb-5 px-5'>
        <div className='flex flex-col gap-3 justify-center items-center'>
          ...
          <p className='text-lg w-1/2 text-center'>
            Are you sure you want to delete {data?.document_level_name} ? You
