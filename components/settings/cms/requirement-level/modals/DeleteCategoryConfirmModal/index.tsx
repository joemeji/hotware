...
export const DeleteCategoryConfirmModal = (props: IDepartmentModal) => {
  const { open, onOpenChange, data, listUrl } = props;

  const deleteCurrency = async () => {
    try {
      const res = await fetch("/api/document/level-categories/delete", {
        method: "POST",
        body: JSON.stringify({
          id: data.document_level_category_id,
        }),
      });
