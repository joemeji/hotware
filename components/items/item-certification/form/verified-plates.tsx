...
export default function VerifiedPlates({
  formData,
  formPlates,
  onChangeFormPlates,
}: any) {
  ...
  return (
    <div className="flex flex-col w-full max-w-[600px] border border-stone-100 mx-auto rounded-xl overflow-hidden p-3 gap-2">
      <span className="flex font-medium py-2 px-3">Verified Plates</span>
      {formData &&
        formData.form_plates.map((item: any, key: number) => (
          <label
            key={key}
            className={cn(
          ...
          >
            <Checkbox
              ...
              onCheckedChange={(isChecked: boolean) =>
                onCheckPlate(key, isChecked)
              }
            />
            <span>{item.form_plates_name}</span>
            {formValue(key) && (
              <Check strokeWidth={3} className="text-blue-500 mr-2 h-5 w-5" />
            )}
          </label>
        ))}
    </div>
