...
export const DeleteLoadingListModal = (props: deleteLoadingList) => {
  ...
  const { data, isLoading, error } = useSWR(
    open ? "/api/loading-list/" + loadingID + "/details" : null,
    fetcher,
    swrOptions
  );
...
export const DeleteLoadingListItemModal = (props: deleteLoadingListItem) => {
  ...
  const urlDelete = isItem
    ? "/api/loading-list/" + loadingID + "/loading-items/item/" + id + "/delete"
    : "/api/loading-list/" +
      loadingID +
      "/loading-items/category/" +
      id +
      "/delete";
  const urlGet = isItem
    ? "/api/loading-list/" +
      ...
      <AlertDialogContent>
        <AlertDialogHeader>
          ...
          <AlertDialogDescription>
            This action cannot be undone. This will permanently delete{" "}
            {data &&
              (isItem ? data.loading_item_name : data.loading_category_name)}
            .
          </AlertDialogDescription>
        </AlertDialogHeader>
        <AlertDialogFooter>
