...
export const previewType = ["Loading List"];
...
const LoadingListPreview = (props: ShippingPreviewProps) => {
  ...
  const pageRoute: any = useMemo(() => {
    return {
      ...
      query: {
        loading_id: router.query.loading_id,
        ...routerQuery,
      },
    };
  }, [routerQuery, router]);
    ...
    if (loading_id) {
      ...
      try {
        const res = await fetch(
          `${baseUrl}/api/loading-list/preview/${loading_id}?${queryParams.toString()}`,
          {
      ...
      <div className="w-[400px] bg-background">
        <div className="p-2 flex flex-col gap-3 h-full">
          <div className="flex items-center gap-2">
            <Link href={`/projects/shipping-list/${loading_id}`}>
              <Button className={cn("p-2.5")} variant="ghost">
                <ArrowLeft className="text-stone-500" />
              </Button>
            </Link>
            <p className="text-base font-medium">Preview LoadingList Details</p>
          </div>
            ...
            >
              {previewType.map((type: any, index: number) => (
                <div key={index} className="flex items-center gap-3">
                  <RadioGroupItem
                    className="data-[state=checked] border-primary/20 cursor-pointer"
                    value={type}
                    id={type + index}
                    checked={queryType == type}
                  />
                  <label className="cursor-pointer" htmlFor={type + index}>
                    {type}
                  </label>
                </div>
              ))}
              ...
              {displayTypes.map((obj: any, key: number) => {
                ...
                return (
                  <div key={key} className="flex items-center gap-3">
                    ...
                    />
                    <label className="cursor-pointer" htmlFor={"id" + key}>
                      {label}
                    </label>
                  </div>
                );
          ...
          <Button
            className={cn("mt-auto", isDownload && loading && "loading")}
            onClick={onDownload}
          >
            Download
          </Button>
...
export default memo(LoadingListPreview);
