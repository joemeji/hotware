...
const AddEquipmentModal = dynamic(
  () => import("@/components/LoadingList/modals/AddEquipmentModal")
);
...
export const LoadingListHeader = () => {
  ...
  const [addItemFromTemplateModal, setAddItemFromTemplateModal] =
    useState(false);
      ...
      <div className="p-4 flex justify-between items-center">
        <p className="font-medium text-lg">
          ...
          <span className="text-base text-slate-400">
            {loadingDetails && "(" + loadingDetails.loading_description + ")"}
          </span>
        </p>
            ...
            {[...loadingHeaderMenu].map((option: any, key: number) => {
              if (loadingDetails !== null) {
                return (
                  <ItemMenu
                    key={key}
                    onClick={(menu: any) => handleAddItemFromTemplate(option)}
                  >
                    {option.icon}
