...
import OrderConfirmationStatusSelect from "@/components/app/order-confirmation-status-select";
...
export const useChangeStatus = ({ defaultValue, onChange }: any) => {
  ...
  const submit = async (status: string) => {
    ...
    const response = await fetch(
      `${baseUrl}/api/projects/orders/change_status/${item}`,
      {
        method: "POST",
    ...
    if (result.success) {
      onChange(result.order_confirmation_id);
    }
