...
function AddTextBlockAttachmentModal(props: AddTextBlockAttachmentModalProps) {
  ...
  const { open, onOpenChange, octb_id, onUpdated } = props;
  ...
  const { mutateDelete, Dialog: DeleteDialog } = useDeleteTextBlockAttachment({
    octb_id,
    onDelete: (octb_attachment_id: string) => {
      const updatedList = items.filter(
        (item) => item.octb_attachment_id !== octb_attachment_id
      );
  ...
  const onSubmitEditForm = async (data: any) => {
    try {
      ...
      let endpoint = `${baseUrl}/api/projects/orders/text_blocks/attachments/${octb_id}/null`;
      ...
      if (isEditMode && editItem) {
        ...
        endpoint = `${baseUrl}/api/projects/orders/text_blocks/attachments/${editItem.octb_id}/${editItem.octb_attachment_id}`;
      }
      ...
      if (json.success) {
        ...
        mutate([
          `/api/projects/orders/text_blocks/attachments/${octb_id}`,
                      ...
                      <tr key={key} className="group">
                        ...
                        <TD className="align-top text-right pe-4">
                          <MoreOption>
                            ...
                            <div
                              onClick={() =>
                                mutateDelete(row.octb_attachment_id)
                              }
                              className="flex items-center p-2 px-3 cursor-pointer gap-3 hover:bg-stone-100 outline-none"
                            >
