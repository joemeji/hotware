...
const ChangeStatusModal = ({ onOpenChange, open }: ChangeStatusModal) => {
  const orderConfirmationStatus = Object.keys(status);
  ...
  const onClickSubmit = async () => {
    ...
    try {
      const res = await fetch(
        `/api/projects/orders/${_order_confirmation_id}/change_status`,
        {
          method: "POST",
      ...
      if (json.success) {
        ...
        setTimeout(() => {
          ...
          mutate(`/api/projects/orders/${_order_confirmation_id}/details`);
        }, 300);
      }
    } catch (err: any) {
      console.log(err);
  ...
  useEffect(() => {
    if (orderConfirmationDetails) {
      setOnStatusChange(orderConfirmationDetails.order_confirmation_status);
    }
          ...
          <div className="px-3 py-3">
            ...
            >
              {orderConfirmationStatus &&
                orderConfirmationStatus.map((statusKey: any, key: any) => (
                  <label
                    htmlFor={statusKey}
                    key={key}
                    tabIndex={0}
