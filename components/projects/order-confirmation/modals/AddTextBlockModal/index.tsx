...
function AddTextBlockModal(props: AddTextBlockModalProps) {
  ...
  const yupSchema = yup.object({
    octb_language: yup.string(),
    octb_title: yup.string(),
    octb_text: yup.string(),
    octb_extra_text: yup.string(),
  });
  ...
  const onSubmitEditForm = async (data: any) => {
    if (!data?.octb_text) return;

    try {
      setIsSubmitting(true);
      const response = await fetch(
        `${baseUrl}/api/projects/orders/text_blocks/${_order_confirmation_id}`,
        {
      ...
      if (json.success) {
        mutate([
          `/api/projects/orders/text_blocks/${_order_confirmation_id}`,
  ...
  const onChangeTextBlock = (textBlock: any) => {
    ...
    setValue("octb_title", textBlock.text_block_title);
    setValue("octb_text", textBlock.text_block_text);
    setValue("octb_extra_text", textBlock.text_block_extra_text);
  };
              ...
              <div className="flex flex-col gap-2">
                ...
                <div>
                  <Combobox
                    ...
                    value="English"
                    onChangeValue={(value) => setValue("octb_language", value)}
                  />
                </div>
              </div>
              <div className="flex flex-col gap-2">
