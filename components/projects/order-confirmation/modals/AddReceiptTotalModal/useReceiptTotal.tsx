...
export const useReceiptTotal = () => {
  ...
  const onChangeValue = (index: number, e: any) => {
    ...
    if (value && !isNaN(value)) {
      onChange(index, { ocrt_value: +value });
    } else {
      // revert changes
      const oldValue = list[index].ocrt_value;
      onChange(index, { ocrt_value: oldValue });
  ...
  const onChangeText = (index: number, e: any) => {
    const value = e?.target?.textContent || null;
    onChange(index, { ocrt_text: value });
  };
  ...
  const handleTextKeyDown = (index: number, e: any) => {
    if (e.keyCode === 13 && !e.shiftKey) {
      ...
      e.preventDefault();
      // Esc
      const oldValue = list[index].ocrt_text;
      onChange(index, { ocrt_text: oldValue });
      e.target.innerHTML = oldValue;
      e.target.blur();
    }
  };
