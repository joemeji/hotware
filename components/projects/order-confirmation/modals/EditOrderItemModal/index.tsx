...
function EditOrderItemModal(props: EditOrderItemModalProps) {
  ...
  const onSubmitEditForm = async (e: any) => {
    ...
    try {
      ...
      const response = await fetch(
        `${baseUrl}/api/projects/orders/items/update/${order_confirmation.oc_item_id}`,
        {
          headers: authHeaders(session?.user?.access_token),
          method: "POST",
          body: JSON.stringify({ data: { oc_item_name: newValue } }),
        }
      );
      ...
      if (json.success) {
        ...
        if (onUpdated) onUpdated(order_confirmation.oc_item_id, newValue);
        ...
        <form onSubmit={onSubmitEditForm}>
          <div className="flex flex-col gap-3 p-4 relative min-h-[560px]">
            <div className="w-full h-[460px]">
              <RichTextEditor
                ref={editorRef}
                value={order_confirmation?.oc_item_name}
              />
            </div>
          </div>
...
export default memo(EditOrderItemModal);
