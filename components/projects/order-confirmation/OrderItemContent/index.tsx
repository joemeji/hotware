...
import AddButtonPopover from "@/components/projects/order-confirmation/OrderDetails/AddButtonPopover";
import { Skeleton } from "@/components/ui/skeleton";
import DetailsHeader from "../OrderDetails/DetailsHeader";
...
import { isOpen } from "@/lib/order";

// modals
const AddEquipmentModal = dynamic(
  () =>
    import("@/components/projects/order-confirmation/modals/AddEquipmentModal")
);
const AddCustomEquipmentModal = dynamic(
  () =>
    import(
      "@/components/projects/order-confirmation/modals/AddCustomOrderItemModal"
    )
);
const EditOrderItemModal = dynamic(
  () =>
    import("@/components/projects/order-confirmation/modals/EditOrderItemModal")
);
const AddReceiptTotalModal = dynamic(
  () =>
    import(
      "@/components/projects/order-confirmation/modals/AddReceiptTotalModal"
    )
);
const AddTextBlockModal = dynamic(
  () =>
    import("@/components/projects/order-confirmation/modals/AddTextBlockModal")
);
const AdditionalHeaderTextModal = dynamic(
  () =>
    import(
      "@/components/projects/order-confirmation/modals/AdditionalHeaderTextModal"
    )
);
...
const EditableInputCell = ({ getValue, row, column, table }: any) => {
  ...
  const submit = () => {
    table.options.meta?.updateData(row.original.oc_item_id, column.id, value);
    setEditable(false);
  };
...
function OrderItemContent({ order_confirmation_id, currency, _data }: any) {
  const isExclusiveVat = _data ? _data.order_confirmation_is_exclusive_vat : 0;
  ...
  const Actions = ({ row, column }: any) => {
    ...
    return (
      <MoreOption>
        ...
        <ItemMenu
          onClick={() => {
            mutateDelete(row.original.oc_item_id);
          }}
        >
          <Trash {...iconProps()} />
  ...
  const columns = [
    ...
    columnHelper.accessor("oc_item_name", {
      header: "Description",
    ...
    columnHelper.accessor("oc_item_quantity", {
      header: "Quantity",
      cell: EditableInputCell,
      meta: {
        width: "8%",
    ...
    columnHelper.accessor("oc_item_price", {
      header: "Unit Price",
      cell: EditableInputCell,
      meta: {
        width: "10%",
    ...
    columnHelper.accessor("oc_item_vat", {
      header: "VAT",
    ...
    columnHelper.accessor("oc_item_discount", {
      header: "Discount",
      cell: EditableInputCell,
      meta: {
        width: "10%",
  ...
  const table = useReactTable({
    ...
    meta: {
      updateData: async (orderItemId: number, columnId: number, value: any) => {
        const response = await fetch(
          `${baseUrl}/api/projects/orders/items/update/${orderItemId}`,
          {
        ...
        const index = items.findIndex(
          (item) => item.oc_item_id === result.item.oc_item_id
        );
  ...
  const { data, isLoading } = useSWR(
    session?.user?.access_token
      ? [
          `/api/projects/orders/items/${order_confirmation_id}`,
          session?.user?.access_token,
        ]
      : null,
    fetchApi,
  ...
  const { data: receiptTotals, isLoading: isLoadingReceiptTotals } = useSWR(
    session?.user?.access_token
      ? [
          `/api/projects/orders/receipt_totals/${order_confirmation_id}`,
          session?.user?.access_token,
        ]
      : null,
    fetchApi,
  ...
  const { data: textBlocks, isLoading: isLoadingTextBlocks } = useSWR(
    session?.user?.access_token
      ? [
          `/api/projects/orders/text_blocks/${order_confirmation_id}`,
          session?.user?.access_token,
        ]
      : null,
    fetchApi,
  ...
  let subtotal = useMemo(() => {
    return (
      items?.reduce(
        (prev: number, item: any) => prev + parseFloat(item.oc_item_line_total),
        0
      ) || 0
    );
  }, [items]);
  ...
  const subtotalWithReceiptTotal = () => {
    ...
    if (items && items.length > 0) {
      items.map((row: any, key: any) => {
        let vat = vats && vats.find((vatTemp) => vatTemp.id == row.oc_item_vat);
        let divisible =
          100 +
          (isExclusiveVat == 1 ? 0 : parseFloat(row.oc_item_vat_percentage));
        let percentage = parseFloat(row.oc_item_vat_percentage) / divisible;
        let amount = parseFloat(row.oc_item_line_total) * percentage;

        if (vat) {
          vat.amount = vat.amount + amount;
        } else if (row.oc_item_vat != 0) {
          vats = [
            ...vats,
            {
              id: row.oc_item_vat,
    ...
    let chargeRows = receiptTotals?.map((receiptTotal: any) => {
      let amount =
        parseInt(receiptTotal.ocrt_type) == 2
          ? (receiptTotal.ocrt_value / 100) * BASE_SUBTOTAL
          : receiptTotal.ocrt_value;
      amount =
        parseInt(receiptTotal.ocrt_is_surcharge) == 1 ? amount : amount * -1;
      subtotal += parseFloat(amount);
      let vat =
        vats && vats.find((vatTemp) => vatTemp.id == receiptTotal.ocrt_vat);
      let divisible =
        100 +
        (isExclusiveVat == 1
          ? 0
          : parseFloat(receiptTotal.ocrt_vat_percentage));

      let percentage = parseFloat(receiptTotal.ocrt_vat_percentage) / divisible;
      let vatAmount = parseFloat(amount) * percentage;

      if (vat) {
        vat.amount = vat.amount + vatAmount;
      } else if (parseInt(receiptTotal.ocrt_vat) != 0) {
        vats = [
          ...vats,
          {
            id: receiptTotal.ocrt_vat,
      ...
      return (
        ...
        >
          ...
          <td className="w-[31%] py-3 px-2 font-bold">
            {receiptTotal.ocrt_text}
          </td>
          <td className="w-[8%]"></td>
          <td className="w-[10%]"></td>
          <td className="w-[10%]">
            {receiptTotal.ocrt_vat == 0 ? "No VAT" : receiptTotal.vat_code}
          </td>
          <td className="w-[10%]"></td>
  ...
  const updatePosition = async (list: any[]) => {
    const items = list.map((list) => list.oc_item_id);
    const response = await fetch(
      `${baseUrl}/api/projects/orders/items/update_positions/${order_confirmation_id}`,
      {
        headers: authHeaders(session?.user?.access_token),
        method: "POST",
  ...
  return (
    <>
      ...
      <AddEquipmentModal
        ...
        order_confirmation_id={router.query.order_confirmation_id}
      ...
      <AddCustomEquipmentModal
        ...
        _order_confirmation_id={router.query.order_confirmation_id}
      />
      ...
      />
      <EditOrderItemModal
        order_confirmation={selectedItem}
        _order_confirmation_id={router.query.order_confirmation_id}
        onOpenChange={(open) => {
          if (!open) {
        ...
        onUpdated={(oc_item_id: any, newVal: any) => {
          const list = [...items];
          const index = list.findIndex((item) => item.oc_item_id == oc_item_id);
          list[index].oc_item_name = newVal;
          setItems(list);
        }}
      />
      {/* End Popover Modal */}
      ...
      <div className="w-3/4 flex flex-col">
        <div className="relative min-h-[calc(100vh-var(--header-height)-40px)]">
          <DetailsHeader
            _order_confirmation_id={router.query.order_confirmation_id?.toString()}
            data={_data}
          />
                  ...
                  >
                    <tbody>
                      {table.getRowModel().rows.map((row, index) => {
                        return (
                          <Draggable
                            key={row.original.oc_item_id}
                            draggableId={row.original.oc_item_id}
                            index={index}
                          >
                            {(provided: any) => (
                              <tr
                    ...
                    {Array.isArray(items) && items.length !== 0 ? (
                      <TextBlocks
                        list={textBlocks}
                        order_confirmation_id={
                          router.query.order_confirmation_id
                        }
                        editable={editable}
                      />
                    ) : null}
...
export default memo(OrderItemContent);
