...
export default function Description({ descriptionWidth, item }: Description) {
  return (
    <div
      ...
      style={{ width: (descriptionWidth || 342) + "px" }}
    >
      {/* <div className="w-[15px] h-[15px] bg-red-300 rounded-full mt-1" /> */}
      <Image
        alt={item.order_confirmation_item_name}
        width={60}
        height={60}
      ...
      <div className="flex flex-col px-2">
        <span
          ...
          dangerouslySetInnerHTML={{
            __html: item.order_confirmation_item_name || "",
          }}
        />
        <span className="text-stone-500">
          {item.order_confirmation_item_country_of_origin}
        </span>
      </div>
    </div>
  );
