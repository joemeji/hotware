...
function AddTextBlockAttachmentModal(props: AddTextBlockAttachmentModalProps) {
  ...
  const { open, onOpenChange, dntb_id, onUpdated } = props;
  ...
  const { mutateDelete, Dialog: DeleteDialog } = useDeleteTextBlockAttachment({
    dntb_id,
    onDelete: (dntb_attachment_id: string) => {
      const updatedList = items.filter(
        (item) => item.dntb_attachment_id !== dntb_attachment_id
      );
  ...
  const onSubmitEditForm = async (data: any) => {
    try {
      ...
      let endpoint = `${baseUrl}/api/projects/deliveries/text_blocks/attachments/${dntb_id}/null`;
      ...
      if (isEditMode && editItem) {
        ...
        endpoint = `${baseUrl}/api/projects/deliveries/text_blocks/attachments/${editItem.dntb_id}/${editItem.dntb_attachment_id}`;
      }
                      ...
                      <tr key={key} className="group">
                        ...
                        <TD className="align-top text-right pe-4">
                          <MoreOption>
                            ...
                            <div
                              onClick={() =>
                                mutateDelete(row.dntb_attachment_id)
                              }
                              className="flex items-center p-2 px-3 cursor-pointer gap-3 hover:bg-stone-100 outline-none"
                            >
