...
function AddReceiptTotalModal(props: AddReceiptTotalModalProps) {
  ...
  const { open, onOpenChange, _delivery_note_id } = props;
  ...
  const { data, isLoading } = useSWR(
    session?.user?.access_token
      ? [
          `/api/projects/deliveries/receipt_totals/${_delivery_note_id}`,
          session?.user?.access_token,
        ]
      : null,
    fetchApi,
  ...
  const onSubmitEditForm = async (e: any) => {
    ...
    const filteredSurcharges = surcharges
      .filter((surcharge) => surcharge.dnrt_value && surcharge.dnrt_text)
      .map((surcharge) => ({ ...surcharge, dnrt_is_surcharge: 1 }));
    const filteredDeductions = deductions
      .filter((deduction) => deduction.dnrt_value && deduction.dnrt_text)
      .map((surcharge) => ({ ...surcharge, dnrt_is_surcharge: 0 }));
    ...
    try {
      ...
      const response = await fetch(
        `${baseUrl}/api/projects/deliveries/receipt_totals/${_delivery_note_id}`,
        {
  ...
  useEffect(() => {
    if (data && data.length) {
      const existingSurcharges = data.filter(
        (receiptTotal: any) => receiptTotal.dnrt_is_surcharge == 1
      );
      const existingDeductions = data.filter(
        (receiptTotal: any) => receiptTotal.dnrt_is_surcharge == 0
      );
                    ...
                    <tr key={index}>
                      <td className="w-[10%] border border-[#e4e7ea] p-2">
                        <ReceiptTotalTypeSelect
                          modal={true}
                          value={surcharge.dnrt_type}
                          onChangeValue={(value) =>
                            onChangeSurcharge(index, { dnrt_type: value })
                          }
                        />
                      </td>
                      <td
                      ...
                      <td className="w-[10%] border border-[#e4e7ea] p-2">
                        <VatSelect
                          modal={true}
                          value={surcharge.dnrt_vat}
                          onChangeValue={(value) =>
                            onChangeSurcharge(index, { dnrt_vat: value })
                          }
                        />
                      </td>
                      <td className="w-[10%] border border-[#e4e7ea] p-2">
                  ...
                  {deductions.map((deduction, index) => (
                    <tr key={index}>
                      <td className="w-[10%] border border-[#e4e7ea] p-2">
                        <ReceiptTotalTypeSelect
                          modal={true}
                          value={deduction.dnrt_type}
                          onChangeValue={(value) =>
                            onChangeDeduction(index, { dnrt_type: value })
                          }
                        />
                      </td>
                      <td
                      ...
                      <td className="w-[10%] border border-[#e4e7ea] p-2">
                        <VatSelect
                          modal={true}
                          value={deduction.dnrt_vat}
                          onChangeValue={(value) =>
                            onChangeDeduction(index, { dnrt_vat: value })
                          }
                        />
                      </td>
                      <td className="w-[10%] border border-[#e4e7ea] p-2">
