...
function AddTextBlockModal(props: AddTextBlockModalProps) {
  ...
  const { open, onOpenChange, _delivery_note_id, onUpdated } = props;
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [selectedTextBlock, setSelectedTextBlock] = useState<any>(0);
  const { mutate } = useSWRConfig();

  const yupSchema = yup.object({
    dntb_language: yup.string(),
    dntb_title: yup.string(),
    dntb_text: yup.string(),
    dntb_extra_text: yup.string(),
  });
  ...
  const onSubmitEditForm = async (data: any) => {
    if (!data?.dntb_text) return;

    try {
      setIsSubmitting(true);
      const response = await fetch(
        `${baseUrl}/api/projects/deliveries/text_blocks/${_delivery_note_id}`,
        {
      ...
      if (json.success) {
        mutate([
          `/api/projects/deliveries/text_blocks/${_delivery_note_id}`,
  ...
  const onChangeTextBlock = (textBlock: any) => {
    ...
    setValue("dntb_title", textBlock.text_block_title);
    setValue("dntb_text", textBlock.text_block_text);
    setValue("dntb_extra_text", textBlock.text_block_extra_text);
  };
              ...
              <div className="flex flex-col gap-2">
                ...
                <div>
                  <Combobox
                    ...
                    value="English"
                    onChangeValue={(value) => setValue("dntb_language", value)}
                  />
                </div>
              </div>
              <div className="flex flex-col gap-2">
