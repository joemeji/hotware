...
function EditDeliveryItemModal(props: EditDeliveryItemModalProps) {
  const { data: session }: any = useSession();
  const { open, onOpenChange, delivery_note, _delivery_note_id, onUpdated } =
    props;
  ...
  const onSubmitEditForm = async (e: any) => {
    ...
    try {
      ...
      const response = await fetch(
        `${baseUrl}/api/projects/deliveries/items/update/${delivery_note.dn_item_id}`,
        {
          headers: authHeaders(session?.user?.access_token),
          method: "POST",
          body: JSON.stringify({ data: { dn_item_name: newValue } }),
        }
      );
      ...
      if (json.success) {
        ...
        if (onUpdated) onUpdated(delivery_note.dn_item_id, newValue);
        ...
        <form onSubmit={onSubmitEditForm}>
          <div className="flex flex-col gap-3 p-4 relative min-h-[560px]">
            <div className="w-full h-[460px]">
              <RichTextEditor
                ref={editorRef}
                value={delivery_note?.dn_item_name}
              />
            </div>
          </div>
...
export default memo(EditDeliveryItemModal);

type EditDeliveryItemModalProps = {
  open?: boolean;
