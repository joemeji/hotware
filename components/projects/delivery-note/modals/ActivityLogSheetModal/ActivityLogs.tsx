...
const historyUri = (_delivery_note_id: any, filter: any = {}) => {
  if (filter.to || filter.from || filter.search) {
    if (filter.page) delete filter.page;
  }
  let searchParams = new URLSearchParams(filter);
  return `/api/document_history/delivery/${_delivery_note_id}?${searchParams}`;
};
  ...
  const { data, isLoading, error, size, setSize, isValidating } =
    useSWRInfinite(
      (index) => {
        ...
        let uri = historyUri(_delivery_note_id, paramsObj);
