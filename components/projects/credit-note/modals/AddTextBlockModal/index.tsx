...
function AddTextBlockModal(props: AddTextBlockModalProps) {
  ...
  const { open, onOpenChange, _credit_note_id, onUpdated } = props;
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [selectedTextBlock, setSelectedTextBlock] = useState<any>(0);
  const { mutate } = useSWRConfig();

  const yupSchema = yup.object({
    cntb_language: yup.string(),
    cntb_title: yup.string(),
    cntb_text: yup.string(),
    cntb_extra_text: yup.string(),
  });
  ...
  const onSubmitEditForm = async (data: any) => {
    if (!data?.cntb_text) return;

    try {
      setIsSubmitting(true);
      const response = await fetch(
        `${baseUrl}/api/projects/credits/text_blocks/${_credit_note_id}`,
        {
      ...
      if (json.success) {
        mutate([
          `/api/projects/credits/text_blocks/${_credit_note_id}`,
  ...
  const onChangeTextBlock = (textBlock: any) => {
    ...
    setValue("cntb_title", textBlock.text_block_title);
    setValue("cntb_text", textBlock.text_block_text);
    setValue("cntb_extra_text", textBlock.text_block_extra_text);
  };
              ...
              <div className="flex flex-col gap-2">
                ...
                <div>
                  <Combobox
                    ...
                    value="English"
                    onChangeValue={(value) => setValue("cntb_language", value)}
                  />
                </div>
              </div>
              <div className="flex flex-col gap-2">
