...
function AddReceiptTotalModal(props: AddReceiptTotalModalProps) {
  ...
  const { data, isLoading } = useSWR(
    session?.user?.access_token
      ? [
          `/api/projects/credits/receipt_totals/${_credit_note_id}`,
          session?.user?.access_token,
        ]
      : null,
    fetchApi,
  ...
  const onSubmitEditForm = async (e: any) => {
    ...
    const filteredSurcharges = surcharges
      .filter((surcharge) => surcharge.cnrt_value && surcharge.cnrt_text)
      .map((surcharge) => ({ ...surcharge, cnrt_is_surcharge: 1 }));
    const filteredDeductions = deductions
      .filter((deduction) => deduction.cnrt_value && deduction.cnrt_text)
      .map((surcharge) => ({ ...surcharge, cnrt_is_surcharge: 0 }));
    ...
    try {
      ...
      const response = await fetch(
        `${baseUrl}/api/projects/credits/receipt_totals/${_credit_note_id}`,
        {
  ...
  useEffect(() => {
    if (data && data.length) {
      const existingSurcharges = data.filter(
        (receiptTotal: any) => receiptTotal.cnrt_is_surcharge == 1
      );
      const existingDeductions = data.filter(
        (receiptTotal: any) => receiptTotal.cnrt_is_surcharge == 0
      );
                    ...
                    <tr key={index}>
                      <td className="w-[10%] border border-[#e4e7ea] p-2">
                        <ReceiptTotalTypeSelect
                          modal={true}
                          value={surcharge.cnrt_type}
                          onChangeValue={(value) =>
                            onChangeSurcharge(index, { cnrt_type: value })
                          }
                        />
                      </td>
                      <td
                      ...
                      <td className="w-[10%] border border-[#e4e7ea] p-2">
                        <VatSelect
                          modal={true}
                          value={surcharge.cnrt_vat}
                          onChangeValue={(value) =>
                            onChangeSurcharge(index, { cnrt_vat: value })
                          }
                        />
                      </td>
                      <td className="w-[10%] border border-[#e4e7ea] p-2">
                  ...
                  {deductions.map((deduction, index) => (
                    <tr key={index}>
                      <td className="w-[10%] border border-[#e4e7ea] p-2">
                        <ReceiptTotalTypeSelect
                          modal={true}
                          value={deduction.cnrt_type}
                          onChangeValue={(value) =>
                            onChangeDeduction(index, { cnrt_type: value })
                          }
                        />
                      </td>
                      <td
                      ...
                      <td className="w-[10%] border border-[#e4e7ea] p-2">
                        <VatSelect
                          modal={true}
                          value={deduction.cnrt_vat}
                          onChangeValue={(value) =>
                            onChangeDeduction(index, { cnrt_vat: value })
                          }
                        />
                      </td>
                      <td className="w-[10%] border border-[#e4e7ea] p-2">
