...
const ChangeStatusModal = ({ onOpenChange, open }: ChangeStatusModal) => {
  ...
  const onClickSubmit = async () => {
    setLoading(true);
    try {
      const res = await fetch(`/api/credits/${_credit_note_id}/change_status`, {
        method: "POST",
      ...
      if (json.success) {
        ...
        setTimeout(() => {
          ...
          mutate(`/api/credits/${_credit_note_id}/details`);
        }, 300);
      }
    } catch (err: any) {
      console.log(err);
  ...
  useEffect(() => {
    if (creditNoteDetails) {
      setOnStatusChange(creditNoteDetails.credit_note_status);
    }
  }, [creditNoteDetails]);
      ...
      <Dialog onOpenChange={onOpenChange} open={open}>
        <DialogContent className="max-w-[450px] p-0 overflow-auto gap-0 ">
          ...
          <div className="px-3 py-3">
            ...
            >
              {creditNoteStatus &&
                creditNoteStatus.map((statusKey: any, key: any) => (
                  <label
                    htmlFor={statusKey}
                    key={key}
                    tabIndex={0}
