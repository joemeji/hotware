...
function EditCreditNoteItemModal(props: EditCreditNoteItemModalProps) {
  const { data: session }: any = useSession();
  const { open, onOpenChange, credit_note, _credit_note_id, onUpdated } = props;
  ...
  const onSubmitEditForm = async (e: any) => {
    ...
    try {
      ...
      const response = await fetch(
        `${baseUrl}/api/projects/credits/items/update/${credit_note.cn_item_id}`,
        {
          headers: authHeaders(session?.user?.access_token),
          method: "POST",
          body: JSON.stringify({ data: { cn_item_name: newValue } }),
        }
      );
      ...
      if (json.success) {
        ...
        if (onUpdated) onUpdated(credit_note.cn_item_id, newValue);
        ...
        <form onSubmit={onSubmitEditForm}>
          <div className="flex flex-col gap-3 p-4 relative min-h-[560px]">
            <div className="w-full h-[460px]">
              <RichTextEditor
                ref={editorRef}
                value={credit_note?.cn_item_name}
              />
            </div>
          </div>
...
export default memo(EditCreditNoteItemModal);

type EditCreditNoteItemModalProps = {
  open?: boolean;
