...
const DetailsHeader = ({ _credit_note_id, data }: DetailsHeaderParams) => {
  ...
  return (
    <div className="flex justify-between py-2 px-3 bg-background rounded-sm mb-2 items-center">
      <div className="flex flex-col gap-1">
        <p className="text-lg font-medium">
          ...
          {data?.credit_note_number && (
            <span>
              {"{ "}
              <span style={{ color: "red" }}>{data.credit_note_number}</span>
              {" }"}
            </span>
          )}
        </p>
      ...
      <DetailsActions _credit_note_id={_credit_note_id} data={data} />
    </div>
  );
};
