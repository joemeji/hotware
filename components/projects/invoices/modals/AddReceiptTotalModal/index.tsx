...
function AddReceiptTotalModal(props: AddReceiptTotalModalProps) {
  ...
  const { data, isLoading } = useSWR(
    session?.user?.access_token
      ? [
          `/api/projects/invoices/receipt_totals/${_invoice_id}`,
          session?.user?.access_token,
        ]
      : null,
    fetchApi,
  ...
  const onSubmitEditForm = async (e: any) => {
    ...
    const filteredSurcharges = surcharges
      .filter((surcharge) => surcharge.irt_value && surcharge.irt_text)
      .map((surcharge) => ({ ...surcharge, irt_is_surcharge: 1 }));
    const filteredDeductions = deductions
      .filter((deduction) => deduction.irt_value && deduction.irt_text)
      .map((surcharge) => ({ ...surcharge, irt_is_surcharge: 0 }));
    ...
    try {
      ...
      const response = await fetch(
        `${baseUrl}/api/projects/invoices/receipt_totals/${_invoice_id}`,
        {
  ...
  useEffect(() => {
    if (data && data.length) {
      const existingSurcharges = data.filter(
        (receiptTotal: any) => receiptTotal.irt_is_surcharge == 1
      );
      const existingDeductions = data.filter(
        (receiptTotal: any) => receiptTotal.irt_is_surcharge == 0
      );
                    ...
                    <tr key={index}>
                      <td className="w-[10%] border border-[#e4e7ea] p-2">
                        <ReceiptTotalTypeSelect
                          modal={true}
                          value={surcharge.irt_type}
                          onChangeValue={(value) =>
                            onChangeSurcharge(index, { irt_type: value })
                          }
                        />
                      </td>
                      <td
                      ...
                      <td className="w-[10%] border border-[#e4e7ea] p-2">
                        <VatSelect
                          modal={true}
                          value={surcharge.irt_vat}
                          onChangeValue={(value) =>
                            onChangeSurcharge(index, { irt_vat: value })
                          }
                        />
                      </td>
                      <td className="w-[10%] border border-[#e4e7ea] p-2">
                  ...
                  {deductions.map((deduction, index) => (
                    <tr key={index}>
                      <td className="w-[10%] border border-[#e4e7ea] p-2">
                        <ReceiptTotalTypeSelect
                          modal={true}
                          value={deduction.irt_type}
                          onChangeValue={(value) =>
                            onChangeDeduction(index, { irt_type: value })
                          }
                        />
                      </td>
                      <td
                      ...
                      <td className="w-[10%] border border-[#e4e7ea] p-2">
                        <VatSelect
                          modal={true}
                          value={deduction.irt_vat}
                          onChangeValue={(value) =>
                            onChangeDeduction(index, { irt_vat: value })
                          }
                        />
                      </td>
                      <td className="w-[10%] border border-[#e4e7ea] p-2">
