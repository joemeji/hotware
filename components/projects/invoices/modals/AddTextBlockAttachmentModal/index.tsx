import {
  ...
  function AddTextBlockAttachmentModal(props: AddTextBlockAttachmentModalProps) {
    ...
    const { open, onOpenChange, itb_id, onUpdated } = props;
    ...
    const { mutateDelete, Dialog: DeleteDialog } = useDeleteTextBlockAttachment({
      itb_id,
      onDelete: (itb_attachment_id: string) => {
        const updatedList = items.filter(
          (item) => item.itb_attachment_id !== itb_attachment_id
        );
    ...
    const onSubmitEditForm = async (data: any) => {
      try {
        ...
        let endpoint = `${baseUrl}/api/projects/invoices/text_blocks/attachments/${itb_id}/null`;
        ...
        if (isEditMode && editItem) {
          ...
          endpoint = `${baseUrl}/api/projects/invoices/text_blocks/attachments/${editItem.itb_id}/${editItem.itb_attachment_id}`;
        }
        ...
        if (json.success) {
          ...
          mutate([
            `/api/projects/invoices/text_blocks/attachments/${itb_id}`,
                        ...
                        <tr key={key} className="group">
                          ...
                          <TD className="align-top text-right pe-4">
                            <MoreOption>
                              ...
                              <div
                                onClick={() =>
                                  mutateDelete(row.itb_attachment_id)
                                }
                                className="flex items-center p-2 px-3 cursor-pointer gap-3 hover:bg-stone-100 outline-none"
                              >
