...
function ViewExportDiscountModal(props: ViewExportDiscountModalProps) {
      ...
      >
        <DialogContent className="max-w-[900px] p-0 overflow-auto gap-0">
          <DialogHeader className="py-2 px-3 flex justify-between flex-row items-center sticky top-0 bg-background z-10">
            <DialogTitle>Invoice Discount/Loss Details</DialogTitle>
          ...
          <table className="m-6">
            ...
            <tbody>
              {data?.invoice ? (
                <tr>
                  <TD>{data.invoice.invoice_discount_amount}</TD>
                  <TD>{data.invoice.invoice_discount_description}</TD>
                  <TD>{data.invoice.discount_exporter}</TD>
                  <TD>{data.invoice.invoice_discount_date}</TD>
                </tr>
              ) : (
                <tr>
...
export default memo(ViewExportDiscountModal);

type ViewExportDiscountModalProps = {
  open?: boolean;
