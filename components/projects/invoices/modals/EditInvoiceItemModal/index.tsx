...
function EditInvoiceItemModal(props: EditInvoiceItemModalProps) {
  const { data: session }: any = useSession();
  const { open, onOpenChange, invoice, _invoice_id, onUpdated } = props;
  ...
  const onSubmitEditForm = async (e: any) => {
    ...
    try {
      ...
      const response = await fetch(
        `${baseUrl}/api/projects/invoices/items/update/${invoice.invoice_item_id}`,
        {
          headers: authHeaders(session?.user?.access_token),
          method: "POST",
          body: JSON.stringify({ data: { invoice_item_name: newValue } }),
        }
      );
      ...
      if (json.success) {
        ...
        if (onUpdated) onUpdated(invoice.invoice_item_id, newValue);
        ...
        <form onSubmit={onSubmitEditForm}>
          <div className="flex flex-col gap-3 p-4 relative min-h-[560px]">
            <div className="w-full h-[460px]">
              <RichTextEditor
                ref={editorRef}
                value={invoice?.invoice_item_name}
              />
            </div>
          </div>
...
export default memo(EditInvoiceItemModal);

type EditInvoiceItemModalProps = {
  open?: boolean;
