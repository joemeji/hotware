...
const ChangeStatusModal = ({ onOpenChange, open }: ChangeStatusModal) => {
  ...
  const onClickSubmit = async () => {
    setLoading(true);
    try {
      const res = await fetch(`/api/invoices/${_invoice_id}/change_status`, {
        method: "POST",
      ...
      if (json.success) {
        ...
        setTimeout(() => {
          ...
          mutate(`/api/invoices/${_invoice_id}/details`);
        }, 300);
      }
    } catch (err: any) {
      console.log(err);
  ...
  useEffect(() => {
    if (invoiceDetails) {
      setOnStatusChange(invoiceDetails.shipping_status);
    }
  }, [invoiceDetails]);
      ...
      <Dialog onOpenChange={onOpenChange} open={open}>
        <DialogContent className="max-w-[450px] p-0 overflow-auto gap-0 ">
          ...
          <div className="px-3 py-3">
            ...
            >
              {invoiceStatus &&
                invoiceStatus.map((statusKey: any, key: any) => (
                  <label
                    htmlFor={statusKey}
                    key={key}
                    tabIndex={0}
