...
function InvoiceItemContent({ invoice_id, currency, _data }: any) {
  const isExclusiveVat = _data ? _data.invoice_is_exclusive_vat : 0;
    ...
    return (
      items?.reduce(
        (prev: number, item: any) =>
          prev + parseFloat(item.invoice_item_line_total),
        0
      ) || 0
    );
  }, [items]);
  ...
  const subtotalWithReceiptTotal = () => {
    ...
    if (items && items.length > 0) {
      items.map((row: any, key: any) => {
        let vat =
          vats && vats.find((vatTemp) => vatTemp.id == row.invoice_item_vat);
        let divisible =
          100 +
          (isExclusiveVat == 1
            ? 0
            : parseFloat(row.invoice_item_vat_percentage));
        let percentage =
          parseFloat(row.invoice_item_vat_percentage) / divisible;
        let amount = parseFloat(row.invoice_item_line_total) * percentage;

        if (vat) {
          vat.amount = vat.amount + amount;
        } else if (row.invoice_item_vat != 0) {
          vats = [
            ...vats,
            {
              id: row.invoice_item_vat,
    ...
    let chargeRows = receiptTotals?.map((receiptTotal: any) => {
      let amount =
        parseInt(receiptTotal.irt_type) == 2
          ? (receiptTotal.irt_value / 100) * BASE_SUBTOTAL
          : receiptTotal.irt_value;
      amount =
        parseInt(receiptTotal.irt_is_surcharge) == 1 ? amount : amount * -1;
      subtotal += parseFloat(amount);
      let vat =
        vats && vats.find((vatTemp) => vatTemp.id == receiptTotal.irt_vat);
      let divisible =
        100 +
        (isExclusiveVat == 1 ? 0 : parseFloat(receiptTotal.irt_vat_percentage));

      let percentage = parseFloat(receiptTotal.irt_vat_percentage) / divisible;
      let vatAmount = parseFloat(amount) * percentage;

      if (vat) {
        vat.amount = vat.amount + vatAmount;
      } else if (parseInt(receiptTotal.irt_vat) != 0) {
        vats = [
          ...vats,
          {
            id: receiptTotal.irt_vat,
      ...
      return (
        ...
        >
          ...
          <td className="w-[31%] py-3 px-2 font-bold">
            {receiptTotal.irt_text}
          </td>
          <td className="w-[8%]"></td>
          <td className="w-[10%]"></td>
          <td className="w-[10%]">
            {receiptTotal.irt_vat == 0 ? "No VAT" : receiptTotal.vat_code}
          </td>
          <td className="w-[10%]"></td>
    ...
    <>
      ...
      <div className="w-3/4 flex flex-col">
        <div className="relative min-h-[calc(100vh-var(--header-height)-40px)]">
          <DetailsHeader
            _invoice_id={router.query.invoice_id?.toString()}
            data={_data}
          />
