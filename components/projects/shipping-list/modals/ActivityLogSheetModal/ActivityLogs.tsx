...
const historyUri = (_shipping_id: any, filter: any = {}) => {
  ...
  return `/api/document_history/shipping/${_shipping_id}?${searchParams}`;
};
  ...
  const { data, isLoading, error, size, setSize, isValidating } =
    useSWRInfinite(
      (index) => {
        ...
        let uri = historyUri(shippingDetails?._shipping_id, paramsObj);
