...
function EditOfferDetails({ id, offer }: any) {
  ...
  const onSubmitForm = async (data: any) => {
    ...
    try {
      ...
      const res = await fetch(`${baseUrl}/api/projects/offers/update/${id}`, {
        method: "POST",
      ...
      if (json.success) {
        toast({
          title: "Offer has been updated successfully.",
          variant: "success",
          duration: 2000,
        });
        setTimeout(() => {
          setLoadingSubmit(false);
          router.push("/projects/offers/" + id);

          // send task emails
          SendTaskEmail("UPDATE_OFFER", session?.user?.access_token);
        }, 300);
      }
    } catch (err: any) {
      setLoadingSubmit(false);
...
export default memo(EditOfferDetails);
