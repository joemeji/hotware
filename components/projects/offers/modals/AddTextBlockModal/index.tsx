...
function AddTextBlockModal(props: AddTextBlockModalProps) {
  ...
  const { open, onOpenChange, _offer_id, onUpdated } = props;
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [selectedTextBlock, setSelectedTextBlock] = useState<any>(0);
  const { mutate } = useSWRConfig();

  const yupSchema = yup.object({
    offertb_language: yup.string(),
    offertb_title: yup.string(),
    offertb_text: yup.string(),
    offertb_extra_text: yup.string(),
  });
  ...
  const onSubmitEditForm = async (data: any) => {
    if (!data?.offertb_text) return;

    try {
      setIsSubmitting(true);
      const response = await fetch(
        `${baseUrl}/api/projects/offers/text_blocks/${_offer_id}`,
        {
      ...
      if (json.success) {
        mutate([
          `/api/projects/offers/text_blocks/${_offer_id}`,
  ...
  const onChangeTextBlock = (textBlock: any) => {
    ...
    setValue("offertb_title", textBlock.text_block_title);
    setValue("offertb_text", textBlock.text_block_text);
    setValue("offertb_extra_text", textBlock.text_block_extra_text);
  };
              ...
              <div className="flex flex-col gap-2">
                ...
                <div>
                  <Combobox
                    ...
                    value="English"
                    onChangeValue={(value) =>
                      setValue("offertb_language", value)
                    }
                  />
                </div>
              </div>
