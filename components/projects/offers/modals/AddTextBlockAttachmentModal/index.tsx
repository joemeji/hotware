...
function AddTextBlockAttachmentModal(props: AddTextBlockAttachmentModalProps) {
  ...
  const { open, onOpenChange, offertb_id, onUpdated } = props;
  ...
  const { mutateDelete, Dialog: DeleteDialog } = useDeleteTextBlockAttachment({
    offertb_id,
    onDelete: (offer_tb_attachment_id: string) => {
      const updatedList = items.filter(
        (item) => item.offer_tb_attachment_id !== offer_tb_attachment_id
      );
  ...
  const onSubmitEditForm = async (data: any) => {
    try {
      ...
      let endpoint = `${baseUrl}/api/projects/offers/text_blocks/attachments/${offertb_id}/null`;
      ...
      if (isEditMode && editItem) {
        ...
        endpoint = `${baseUrl}/api/projects/offers/text_blocks/attachments/${editItem.offer_tb_id}/${editItem.offer_tb_attachment_id}`;
      }
      ...
      if (json.success) {
        ...
        mutate([
          `/api/projects/offers/text_blocks/attachments/${offertb_id}`,
                      ...
                      <tr key={key} className="group">
                        ...
                        <TD className="align-top text-right pe-4">
                          <MoreOption>
                            ...
                            <div
                              onClick={() =>
                                mutateDelete(row.offer_tb_attachment_id)
                              }
                              className="flex items-center p-2 px-3 cursor-pointer gap-3 hover:bg-stone-100 outline-none"
                            >
