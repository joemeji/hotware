...
function AdditionalHeaderTextModal(props: AdditionalHeaderTextModalProps) {
  ...
  const { data, isLoading } = useSWR(
    [`/api/projects/offers/details/${_offer_id}`, session?.user?.access_token],
    fetchApi
  );
  ...
  const onSubmitEditForm = async (e: any) => {
    ...
    try {
      ...
      const response = await fetch(
        `${baseUrl}/api/projects/offers/update_header_text/${_offer_id}`,
        {
        ...
        <form onSubmit={onSubmitEditForm}>
          <div className="flex flex-col gap-3 p-4 relative min-h-[560px]">
            <div className="w-full h-[460px]">
              <RichTextEditor
                ref={editorRef}
                value={data?.offer_additional_header_text || ""}
              />
            </div>
          </div>
