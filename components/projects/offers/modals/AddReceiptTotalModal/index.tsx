...
function AddReceiptTotalModal(props: AddReceiptTotalModalProps) {
  ...
  const { open, onOpenChange, _offer_id } = props;
  ...
  const { data, isLoading } = useSWR(
    session?.user?.access_token
      ? [
          `/api/projects/offers/receipt_totals/${_offer_id}`,
          session?.user?.access_token,
        ]
      : null,
    fetchApi,
  ...
  const onSubmitEditForm = async (e: any) => {
    ...
    const filteredSurcharges = surcharges
      .filter((surcharge) => surcharge.offerrt_value && surcharge.offerrt_text)
      .map((surcharge) => ({ ...surcharge, offerrt_is_surcharge: 1 }));
    const filteredDeductions = deductions
      .filter((deduction) => deduction.offerrt_value && deduction.offerrt_text)
      .map((surcharge) => ({ ...surcharge, offerrt_is_surcharge: 0 }));
    ...
    try {
      ...
      const response = await fetch(
        `${baseUrl}/api/projects/offers/receipt_totals/${_offer_id}`,
        {
  ...
  useEffect(() => {
    if (data && data.length) {
      const existingSurcharges = data.filter(
        (receiptTotal: any) => receiptTotal.offerrt_is_surcharge == 1
      );
      const existingDeductions = data.filter(
        (receiptTotal: any) => receiptTotal.offerrt_is_surcharge == 0
      );
                    ...
                    <tr key={index}>
                      <td className="w-[10%] border border-[#e4e7ea] p-2">
                        <ReceiptTotalTypeSelect
                          modal={true}
                          value={surcharge.offerrt_type}
                          onChangeValue={(value) =>
                            onChangeSurcharge(index, { offerrt_type: value })
                          }
                        />
                      </td>
                      <td
                      ...
                      <td className="w-[10%] border border-[#e4e7ea] p-2">
                        <VatSelect
                          modal={true}
                          value={surcharge.offerrt_vat}
                          onChangeValue={(value) =>
                            onChangeSurcharge(index, { offerrt_vat: value })
                          }
                        />
                      </td>
                      <td className="w-[10%] border border-[#e4e7ea] p-2">
                  ...
                  {deductions.map((deduction, index) => (
                    <tr key={index}>
                      <td className="w-[10%] border border-[#e4e7ea] p-2">
                        <ReceiptTotalTypeSelect
                          modal={true}
                          value={deduction.offerrt_type}
                          onChangeValue={(value) =>
                            onChangeDeduction(index, { offerrt_type: value })
                          }
                        />
                      </td>
                      <td
                      ...
                      <td className="w-[10%] border border-[#e4e7ea] p-2">
                        <VatSelect
                          modal={true}
                          value={deduction.offerrt_vat}
                          onChangeValue={(value) =>
                            onChangeDeduction(index, { offerrt_vat: value })
                          }
                        />
                      </td>
                      <td className="w-[10%] border border-[#e4e7ea] p-2">
