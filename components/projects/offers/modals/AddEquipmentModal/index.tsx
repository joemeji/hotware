...
function AddEquipmentModal(props: AddEquipmentModalType) {
  ...
  const onMoveItemToCategory = useCallback((selectedCategory: any) => {
  ...
  const onClickSaveSelectedEquipment = async () => {
    ...
    const uri = '/api/projects/offers/items/create/';

    try {
      const res = await fetch(baseUrl + uri + offer_id, options);
      const json = await res.json();
      if (json && json.success) {
        mutate([`/api/projects/offers/items/${offer_id}`, session?.user?.access_token]);
        setTimeout(() => {
