...
export const EditScopeWork = (props: EditScopeWorkProps) => {
  const access_token = useContext(AccessTokenContext);
  const { open, onOpenChange, project, scope, onSuccess } = props;
  ...
  const {
    ...
    defaultValues: {
      project_scope_name: scope.project_scope_name,
    },
  });
  ...
  const handleFormSubmit = async (data: any) => {
    try {
      ...
      const res = await fetch(
        `${baseUrl}/api/projects/${project._project_id}/scope/update/${scope.project_scope_id}`,
        {
          method: "POST",
