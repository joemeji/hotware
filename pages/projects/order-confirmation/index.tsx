...
export default function OrderConfirmation({ access_token }: any) {
  ...
  const dashOffset = useMemo(() => {
    if (list && list.order_confirmation) {
      const row = list.order_confirmation[0];
      const completion = Math.min(
        100,
        Math.max(0, row.percentage_completion || 0)
      );

      // Ensure the dashOffset never goes above 400 (full fill)
      return Math.min(400, 400 - (400 * completion) / 100);
    }

    return 400; // Default dash offset if data is not available
  }, [list]);
      ...
      <div className="p-[20px] w-full max-w-[1600px] mx-auto">
        <div className="bg-white w-full rounded-sm shadow-sm">
          <div className="flex justify-between p-4 items-center">
            <p className="text-xl flex font-medium">Order Confirmations</p>
            ...
            <Link href="/projects/order-confirmation/create">
              <Button>New Order Confirmation</Button>
            </Link>
          </div>
