...
export default function CreditNote({ access_token, credit_note_number }: any) {
  ...
  const { data, isLoading, mutate } = useSWR(
    [
      `/api/projects/credits/${credit_note_number}/list?${queryString}`,
      access_token,
    ],
    fetchApi,
    {
        ...
        <div className="bg-white w-full rounded-sm shadow-sm">
          <div className="flex justify-between p-4 items-center">
            <p className="text-xl flex font-medium">
              Credit Notes for {list?.credit_note_number}
            </p>
                      ...
                      >
                        <CoinsIcon
                          className={`w-[18px] h-[18px] ${
                            row.credit_note_is_booked == 1
                              ? "text-red-500"
                              : "text-green-500"
                          }`}
                        />
                        <span className="text-sm font-medium">
                          {row.credit_note_is_booked == 1 ? "Unbook" : "Book"}{" "}
                          (Accounting)
                        </span>
                      </div>
                      <Separator className="my-2" />
...
export async function getServerSideProps(context: GetServerSidePropsContext) {
  ...
  return {
    props: {
      access_token: token,
      credit_note_number: context.params.credit_note_id,
    },
  };
}
